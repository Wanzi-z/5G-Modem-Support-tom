...
set_lockband_all()
{
    ...
    res=$(sh ${SCRIPT_DIR}/modem_at.sh $at_port $set_lockband_command)
}
...
lockcell_all(){
    if [ -z "$pci" ] && [ -z "$arfcn" ]; then
        ...
        res1=$(sh ${SCRIPT_DIR}/modem_at.sh $at_port $unlockcell)
        res=$res1
    else
        ...
        if [ "$rat" -eq 1 ]; then
            res=$(sh ${SCRIPT_DIR}/modem_at.sh $at_port $lockpci_nr)
        elif [ "$rat" -eq 0 ]; then
            res=$(sh ${SCRIPT_DIR}/modem_at.sh $at_port $lockpci_lte)
        fi
    fi
}
